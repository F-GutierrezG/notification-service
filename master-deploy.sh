...
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=NotificationsServiceSwagger -d --restart always -e 'API_URL=definitions/swagger.yml' --name notifications-swagger --network notifications-service-network --ip 172.23.0.4 $REGISTRY_REPO/$SWAGGER:$TAG"
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=NotificationsService -d --restart always -e 'NODE_ENV=production' -e 'DATABASE_URL=postgres://$DATABASE_PROD_USER:$DATABASE_PROD_PASSWORD@$DATABASE_PROD_URL:$DATABASE_PROD_PORT/$DATABASE_PROD_NAME' --name notifications --network notifications-service-network --ip 172.23.0.2 $REGISTRY_REPO/$NOTIFICATIONS:$TAG"
